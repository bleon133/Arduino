...
char key; //Entrada
double Num=0; //Entrada
double Resultado=0; //Entrada
...
void loop() {
   ...
   if (pantalla==0){ //Cambio de pantalla
    lcd.setCursor(0,0);
    lcd.write("Numero"); //Salida
    numero();
    Num=valor; //Proceso
    lcd.setCursor(0,1);
    lcd.print(Num); //Salida
   }

   if (Num<10){
      Resultado = pow(Num,4); //Proceso
   }

   if (Num>=10 && Num <=100){
      Resultado = Num*4; //Proceso
   }
   ...
   if (pantalla==1){ //Cambio de pantalla
    lcd.setCursor(0,0);
    lcd.write("El resultado es:"); //Salida
    lcd.setCursor(0,1);
    lcd.print(Resultado); //Salida
   }  
}
